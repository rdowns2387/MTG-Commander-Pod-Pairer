...
const MatchHistory = () => {
  ...
  const renderCommanderBg = (imgUrl) => {
    return {
      backgroundImage: `linear-gradient(180deg,rgba(0, 0, 0, 0.25) 0%, rgba(0, 0, 0, 1) 100%), url(${imgUrl})`,
      backgroundPosition: "center",
      backgroundSize: "cover",
    };
  };

  const renderWinnerBg = (imgUrl) => {
    return {
      backgroundImage: `linear-gradient(180deg,rgba(224, 181, 88, 0) 0%, rgba(224, 181, 88, .75) 100%), url(${imgUrl})`,
      backgroundPosition: "center",
      backgroundSize: "cover",
    };
  };
              ...
              <div className="match-players">
                {match.pod.players.map((player) => (
                  <div
                    ...
                    style={
                      (player.wonGame &&
                        renderWinnerBg(player?.commander?.commanderImgUrl)) ||
                      renderCommanderBg(player?.commander?.commanderImgUrl)
                    }
                    className="match-player fg-1 2"
                  >
                    <div className="fg-1">
                      <p>
                    ...
                    <div className="fg-1">
                      ...
                      {player.user._id !== currentUser.id && (
                        <button
                          className="btn btn-sm btn-secondary fg-1"
                          onClick={() =>
                            openRatingModal(match.pod._id, player.user)
                          }
                        >
                          Rate Player
                        </button>
            ...
            <div className="rating-stars">
              {[1, 2, 3, 4, 5].map((rating) => (
                <span
                  key={rating}
                  className={`star ${
                    rating <= selectedRating ? "star-filled" : "star-empty"
                  }`}
